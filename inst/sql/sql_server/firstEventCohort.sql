...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (@condition_concept_ids)and invalid_reason is null
...
-- End Condition Occurrence Criteria
  ...
  WHERE DATEADD(day,@prior_observation_period,OP.OBSERVATION_PERIOD_START_DATE) <= E.START_DATE AND DATEADD(day,0,E.START_DATE) <= OP.OBSERVATION_PERIOD_END_DATE
) P
WHERE P.ordinal = 1
-- End Primary Events
