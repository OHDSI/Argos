SELECT cohort_definition_id, YEAR(cohort_start_date) as cohort_start_year,DATEDIFF(@min_date_unit,COHORT_START_DATE,visit.visit_start_date) as date_unit, visit.visit_concept_id, COUNT(visit.visit_occurrence_id) AS visit_count, COUNT(DISTINCT cohort.subject_id) AS subject_count, SUM(cost.paid_by_payer) AS paid_by_payer_sum, SUM(cost.paid_by_patient) AS paid_by_patient_sum, SUM(cost.total_charge) AS total_charge_sum--,concept.concept_name as visit_concept_name
    ...
    GROUP BY cohort_definition_id, YEAR(cohort_start_date),DATEDIFF(@min_date_unit,COHORT_START_DATE,visit.visit_start_date), visit.visit_concept_id --, concept.concept_name
    ;