...
```{r}
library(dplyr)
cohort_1 <- readRDS(file.path(outputFolder,"incidenceData_cohortId_1.rds") )
df_1 <- cohort_1$data %>% group_by(cohortStartYear,genderConceptId) %>% summarise(cohortCount = sum(aggregatedNum) ) %>% mutate(genderConceptId = case_when(genderConceptId == 8532 ~ "women", genderConceptId == 8507 ~ "men") )
t<- reshape2::dcast(df_1,df_1$genderConceptId~df_1$cohortStartYear)
colnames(t)[1] <- "gender"
knitr::kable(t)
```
...
```{r}
library(dplyr)
cohort_1 <- readRDS(file.path(outputFolder,"incidenceData_cohortId_3.rds") )
df_1 <- cohort_1$data %>% group_by(cohortStartYear,genderConceptId) %>% summarise(cohortCount = sum(aggregatedNum) ) %>% mutate(genderConceptId = case_when(genderConceptId == 8532 ~ "women", genderConceptId == 8507 ~ "men") )
t<- reshape2::dcast(df_1,df_1$genderConceptId~df_1$cohortStartYear)
colnames(t)[1] <- "gender"
knitr::kable(t)
...
```{r}
library(dplyr)
cohort_1 <- readRDS(file.path(outputFolder,"incidenceData_cohortId_2.rds") )
df_1 <- cohort_1$data %>% group_by(cohortStartYear,genderConceptId) %>% summarise(cohortCount = sum(aggregatedNum) ) %>% mutate(genderConceptId = case_when(genderConceptId == 8532 ~ "women", genderConceptId == 8507 ~ "men") )
t<- reshape2::dcast(df_1,df_1$genderConceptId~df_1$cohortStartYear)
colnames(t)[1] <- "gender"
knitr::kable(t)
